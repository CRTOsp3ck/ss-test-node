...
// account id, block number, hash, are custom types are available
// specific to Substrate (can be used through the imported modules)
pub use pallet::*;
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn mint(owner: &T::AccountId, unique_id: [u8; 16], color: Color, ) -> Result<[u8; 16], DispatchError> {
			// Create a new object
		...
		pub fn do_buy_collectible(unique_id: [u8; 16], to: T::AccountId, bid_price: BalanceOf<T>, ) -> DispatchResult {
